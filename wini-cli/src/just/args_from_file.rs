use {
    crate::{
        init::err::InitError,
        just::{MinimalJustfile, MinimalParam, MinimalRecipe},
    },
    pub_just::{Compiler, Loader},
    std::{collections::HashMap, ffi::OsString, path::Path},
};

pub fn arguments_from_justfile_path(path: &Path) -> Result<MinimalJustfile, InitError> {
