use {
    crate::ast,
    proc_macro2::{Delimiter, Ident, Literal, Spacing, Span, TokenStream, TokenTree},
    proc_macro_error::{abort, abort_call_site, emit_error, SpanRange},
                ...
                match self.peek() {
                    ...
                    _ => {
                        match self.next() {
                            ...
                            {
                                let body = self
                                    .block(group.stream(), SpanRange::single_span(group.span()));
                                segments.push(ast::Special {
                                    at_span: SpanRange::single_span(at_span),
